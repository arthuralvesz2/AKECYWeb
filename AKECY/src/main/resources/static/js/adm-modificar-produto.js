document.getElementById('modificarProdutoBtn').addEventListener('click', function(event) {
    ...
    var tamanhosInput = document.getElementById('tamanhos');
    if (tamanhosInput.value.trim() === "") {
        alert("Preencha os tamanhos disponíveis");
        event.preventDefault();
        return;
    }
	...
	var descricaoCompletaInput = document.getElementById('descricaoCompleta'); // Certifique-se que este é o ID correto
	if (descricaoCompletaInput.value.trim() === "") {
	    alert("Preencha a descrição completa do produto");
	    event.preventDefault();
	    return;
	}
