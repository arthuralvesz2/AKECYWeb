...
function salvarLinha(link, idUsuario) {
	...
	data.statusUsuario = statusAtual === 'ATIVO' ? 'ATIVO' : 'INATIVO';

	var nivelAtual = row.querySelector('.nivel-cell .nivel-btn').getAttribute('data-nivel');
	data.nivelAcesso = nivelAtual;

	// Verificar duplicatas antes de enviar a requisição
	fetch('/AKECY/ADM/verificar-duplicatas', {
		method: 'POST',
		headers: {
		...
		.then(result => {
			if (result.duplicatas) {
				let mensagens = [];
				if (result.duplicatas.email) {
					mensagens.push("O email já está cadastrado para outro usuário.");
				}
				if (result.duplicatas.telefone) {
					mensagens.push("O telefone já está cadastrado para outro usuário.");
				}
				if (result.duplicatas.cpf) {
					mensagens.push("O CPF já está cadastrado para outro usuário.");
				}
				if (mensagens.length > 0) {
					alert(mensagens.join("\n"));
					return; // Impede o envio da requisição de salvar
				}
			}
